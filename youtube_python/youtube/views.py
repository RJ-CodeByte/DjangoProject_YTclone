...
def deletevideo(request,id):
    dvideo=Video.objects.get(id=id)
    dvideo.delete()
    return redirect('channel',user=request.user)
...
def search(request):
    ...
    data=Video.objects.filter(title__icontains=vname)
    ytvideos=YoutubeVideo.objects.filter(videoname__icontains=vname)
    channel = False
    print(request.user.username)
    if request.user.username != "":
        try:
    ...
    return render(request,"search.html", {'channel':channel,"d":"Sorry, Video Not found",'data':data})
